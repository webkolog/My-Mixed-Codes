...
'90 ' + SUBSTRING(mm.TelNo,1,3) + ' ' + SUBSTRING(mm.TelNo,4,7)
ELSE
SUBSTRING(mm.TelNo,1,2) + ' ' + SUBSTRING(mm.TelNo,3,3) + ' ' + SUBSTRING(mm.TelNo,6,7)
END AS [Telefon Numarasý]
FROM MyMusteriler mm LEFT JOIN MyIller mi ON mi.ILID = SUBSTRING(REPLACE(SUBSTRING(mm.TelNo,1,2),'90','') + SUBSTRING(mm.TelNo,3,LEN(mm.TelNo)-7),1,3)
...
CONVERT(NVARCHAR(50), (DATEDIFF(SECOND, GETDATE(), a.[Gidiþ Tarihi])/86400)) + ' gün, ' + 
CONVERT(NVARCHAR(50), ((DATEDIFF(SECOND, GETDATE(), a.[Gidiþ Tarihi])%86400)/3600)) + ' saat, ' +
CONVERT(NVARCHAR(50), ((DATEDIFF(SECOND, GETDATE(), a.[Gidiþ Tarihi])%86400)%3600)/60) + ' dakika, ' +
CONVERT(NVARCHAR(50), ((DATEDIFF(SECOND, GETDATE(), a.[Gidiþ Tarihi])%86400)%3600)%60) + ' saniyedir yolda '
ELSE
'(Yolda Deðil)'
